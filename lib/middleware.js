...
module.exports = function cacheRenderer(nuxt, config) {
    ...
    function preprocessCacheKey(input, context) {
        if(config.preprocessKey && config.preprocessKey instanceof Function) {
            return config.preprocessKey(input, context);
        }
        return input;
    }
        ...
        function renderSetCache(){
            return renderRoute(route, context)
                .then(function(result) {
                    if (!result.error) {
                        cache.set(preprocessCacheKey(route, context), serialize(result));
                    }
                    return result;
                });
        }

        return cache.get(preprocessCacheKey(route, context))
