...
module.exports = function cacheRenderer(nuxt, config) {
    ...
    function preprocessCacheKey(input) {
        if(config.preprocessKey && config.preprocessKey instanceof Function) {
            return config.preprocessKey(input);
        }
        return input;
    }
        ...
        function renderSetCache(){
            return renderRoute(route, context)
                .then(function(result) {
                    if (!result.error) {
                        cache.set(preprocessCacheKey(route), serialize(result));
                    }
                    return result;
                });
        }

        return cache.get(preprocessCacheKey(route))
